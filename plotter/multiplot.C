...
bool multiplot() {
  ...
  for (unsigned int ifile=0; ifile < filename.size(); ++ifile) {
	...
	//	h_th1f[ifile][iplot]->Scale(1./temp->Integral());
      }
...
/*
    TFile *fake = new TFile("/uscms_data/d3/byates/CMSSW_5_3_14_patch2/src/muon/SingleMu_FR.root","READ");
    TH1F *tmp = (TH1F*)fake->Get("TotalFakes/h_secondMuonPt_tf_TOTDY");
    tmp->SetLineColor(kBlack);
    tmp->SetFillColor(kCyan);
*/
    //hstack->Add(tmp);
	...
	//h_th1f[ifile][iplot]->Draw( "samep9X0" );
        //TString txtName = TString("mplot/") + TString(hist1d[iplot].name()) + "_integral.txt";
        /*
        TString txtName = TString("mplot/integrals.csv");
        std::ofstream ofs(txtName, std::ofstream::out | std::ofstream::app);
        ofs << filename[ifile] + ",";
        ofs << legendname[ifile] + ",";
        ofs << h_th1f[ifile][iplot]->GetName() << ",";
        ofs << h_th1f[ifile][iplot]->Integral() << std::endl;
        ofs.close();
        */
      }
      ...
      for (unsigned int ifile = 1; ifile < filename.size(); ifile++)
	{
	  if ( type[ifile] == "signal" ) //|| type[ifile] == "signal_mc") 
		...
		*/
	      if (h_th1f[ifile][iplot]->Integral(i,hdevMaj->GetNbinsX())+htotal->Integral(i,hdevMaj->GetNbinsX()) > 0)
		hdevMaj->SetBinContent(i,h_th1f[ifile][iplot]->Integral(i,hdevMaj->GetNbinsX())/sqrt(htotal->Integral(i,hdevMaj->GetNbinsX()) ) );
	      hdevMaj->SetBinError(i,0);
	      if (h_th1f[ifile][iplot]->Integral(1,i)+htotal->Integral(1,i) > 0)
		hdevMin->SetBinContent(i,h_th1f[ifile][iplot]->Integral(1,i)/sqrt(htotal->Integral(1,i) ) );
	    }
	}
      hdevMaj->SetLineColor(kBlack);
      hdevMaj->GetYaxis()->SetTitle( "#frac{S}{#sqrt{B}}" );
      ...
      hdevMin->GetYaxis()->SetRangeUser(0., hdevMin->GetBinContent(hdevMin->GetMaximumBin())*1.2 );
    ...
    TLegend *legend = new TLegend(.65, 0.945 - legendname.size()*0.065, .93, 0.91); // xmin ymin xmax ymax (0.65, 0.945 - (size), 0.95, 945)
    ...
    */
    ...
    if (hist1d[iplot].normalize()) can->Print(TString("mplot/") + TString(hist1d[iplot].name()) + "_norm.pdf");
             ...
             can->Print(TString("mplot/") + TString(hist1d[iplot].name()) + "_log.eps"); }
    else {                          can->Print(TString("mplot/") + TString(hist1d[iplot].name()) + ".pdf");
                               can->Print(TString("mplot/") + TString(hist1d[iplot].name()) + ".pdf"); }
