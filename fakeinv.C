...
void fakeinv(TString version="FR"){
...
/*
h_sigSS = (TH1F*)sigSS.at(0)->Get("NoJets_SS/h_NoJets_SS_invpt");
h_sigOS = (TH1F*)sigSS.at(0)->Get("NoJets_OS/h_NoJets_OS_invpt");
h_fake = (TH1F*)sigSS.at(0)->Get("TotalFakes/h_inv_secondMuonPt_tf_TOTDY");
h_bgSS = (TH1F*)MC.at(0)->Get("NoJets_SS/h_NoJets_SS_invpt");
h_bgOS = (TH1F*)MC.at(0)->Get("NoJets_OS/h_NoJets_OS_invpt");
*/

// barrel |eta| < 0.8, transition 0.8 < |eta| < 1.2, endcap |eta| > 1.2
TH2F *tmp_SS = (TH2F*)sigSS.at(0)->Get("NoJets_SS/h_NoJets_SS_invpT");
h_sigSS = (TH1F*)tmp_SS->ProjectionX("h_NoJets_SS_invpt_sig",43,59);
TH2F *tmp_OS = (TH2F*)sigSS.at(0)->Get("NoJets_OS/h_NoJets_OS_invpT");
h_sigOS = (TH1F*)tmp_OS->ProjectionX("h_NoJets_OS_invpt_sig",43,59);
TH2F *tmp_SS = (TH2F*)sigSS.at(0)->Get("TotalFakes/h_tf_TOTDY_invpT");
//h_fake = (TH1F*)sigSS.at(0)->Get("TotalFakes/h_inv_secondMuonPt_tf_TOTDY");
h_fake = (TH1F*)tmp_SS->ProjectionX("h_fake_invpt",43,59);
tmp_SS = (TH2F*)MC.at(0)->Get("NoJets_SS/h_NoJets_SS_invpT");
h_bgSS = (TH1F*)tmp_SS->ProjectionX("h_NoJets_SS_invpt",43,59);
tmp_OS = (TH2F*)MC.at(0)->Get("NoJets_OS/h_NoJets_OS_invpT");
...
/*
  TH1F *h_tmp = (TH1F*)MC.at(i)->Get("NoJets_SS/h_NoJets_SS_invpt");
  h_bgSS->Add(h_tmp,1);
  h_tmp = (TH1F*)MC.at(i)->Get("NoJets_OS/h_NoJets_OS_invpt");
  h_bgOS->Add(h_tmp,1);
*/
  TH2F *h_tmp = (TH2F*)MC.at(i)->Get("NoJets_SS/h_NoJets_SS_invpT");
  h_bgSS->Add(h_tmp->ProjectionX("h_NoJets_SS_invpt"+i,43,59),1);
  h_tmp = (TH2F*)MC.at(i)->Get("NoJets_OS/h_NoJets_OS_invpT");
  h_bgOS->Add(h_tmp->ProjectionX("h_NoJets_OS_invpt"+i,43,59),1);
}
...
h_flip->Draw();
gStyle->SetOptStat(0);
c.SaveAs("Charge_Filp_Rate_invpt_"+version+".pdf");
h_flip->SetAxisRange(10,10e3, "X");
//gPad->SetLogx();
gPad->SetLogy(0);
TGaxis::SetMaxDigits(2);
c.SaveAs("Charge_Filp_Rate_invpt_log_"+version+".pdf");
c.SaveAs("Charge_Filp_Rate_invpt_log_"+version+".eps");

TFile *out = new TFile("SingleMu_"+version+".root","UPDATE");
h_flip->Write("h_Flip_Rate_invpt",TObject::kOverwrite);
out->Close();
